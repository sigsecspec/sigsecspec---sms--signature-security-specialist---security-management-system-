...
import { 
  ...
  Send, ChevronRight, Info, Shield, Check, AlertTriangle,
  Upload, PenTool, Hash, Activity
} from 'lucide-react';
...
const SECTIONS = [
  ...
  { id: 10, title: 'Consent', icon: PenTool },
  { id: 11, title: 'Submit', icon: Send },
];

const OperationsApplicationForm: React.FC<OperationsFormProps> = ({ user, onSubmit }) => {
  ...
  const [formData, setFormData] = useState<any>({
    ...
    skill_techProf: '',
    skill_comm: '',
    skill_problemSolving: '',
    skill_decisionMaking: '',
    skill_leadership: '',

    // Section 5: Company Understanding
    whyOps: '',
    ...
    ackPartOwnership: false,
    ackGuardTraining: false,
    ackOpsTraining: false,
    ackRules: false,
    ackResponsibility: false,
    digitalSig: '',
  ...
  const renderSection = () => {
    switch (activeSection) {
        ...
        return (
          <div className="space-y-6 animate-fade-in-up">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div><InputLabel>State Security License #</InputLabel><InputField name="stateSecurityLicense" value={formData.stateSecurityLicense} onChange={handleChange} placeholder="If applicable" /></div>
            </div>
          </div>
        );
      case 2: // Experience
          ...
          <div className="space-y-8 animate-fade-in-up">
             <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><InputLabel>Years of Security Experience</InputLabel><InputField name="secExpYears" type="number" value={formData.secExpYears} onChange={handleChange} /></div>
                    <div><InputLabel>Years in Management</InputLabel><InputField name="mgmtExpYears" type="number" value={formData.mgmtExpYears} onChange={handleChange} /></div>
                    ...
                    <div><InputLabel>Roles Held</InputLabel><InputField name="rolesHeld" value={formData.rolesHeld} onChange={handleChange} /></div>
                    <div><InputLabel>Team Sizes Managed</InputLabel><InputField name="teamSizes" value={formData.teamSizes} onChange={handleChange} /></div>
                </div>
             </div>
          </div>
        );
      case 3: // Education
        return (
          <div className="space-y-6 animate-fade-in-up">
             <div><InputLabel>Highest Level of Education</InputLabel><InputField name="highestEdu" value={formData.highestEdu} onChange={handleChange} /></div>
             <div><InputLabel>Professional Certifications</InputLabel><textarea name="profCerts" value={formData.profCerts} onChange={handleChange} className="w-full bg-brand-800 border border-brand-700 rounded p-3 text-white h-24" placeholder="e.g. CPP, PSP, etc." /></div>
          </div>
        );
      case 4: // Skills
        return (
          <div className="space-y-8 animate-fade-in-up">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                {renderSkillRating('Operations Management', 'skill_opsMgmt')}
      ...
      case 5: // Company Fit
        return (
          <div className="space-y-6 animate-fade-in-up">
             <div><InputLabel>Why are you interested in an Operations position? *</InputLabel><textarea name="whyOps" value={formData.whyOps} onChange={handleChange} className="w-full bg-brand-800 border border-brand-700 rounded p-3 text-white h-32" required /></div>
          </div>
        );
      case 6: // Availability
        return (
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div className="flex gap-4 mt-2">
                        <label className="flex items-center space-x-2"><input type="radio" name="availability" value="Full-Time" checked={formData.availability === 'Full-Time'} onChange={handleChange} /> <span>Full-Time</span></label>
                        <label className="flex items-center space-x-2"><input type="radio" name="availability" value="Part-Time" checked={formData.availability === 'Part-Time'} onChange={handleChange} /> <span>Part-Time</span></label>
                    </div>
                </div>
             </div>
          </div>
      ...
      case 8: // Documents
        return (
          <div className="space-y-6 animate-fade-in-up">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div><InputLabel>Resume Upload *</InputLabel><FileUpload label="Resume" /></div>
...
export default OperationsApplicationForm;
